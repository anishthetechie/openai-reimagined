          ...
          <nav className="hidden md:flex items-center gap-1">
            {links.map((l) => (
              <a
                key={l.name}
                href={l.href}
                className="relative px-3 py-2 text-sm text-ink-300 hover:text-white transition-colors group"
              >
                {l.name}
                <span className="absolute left-3 right-3 -bottom-px h-px bg-gradient-to-r from-accent-violet to-accent-cyan scale-x-0 group-hover:scale-x-100 transition-transform origin-left" />
              </a>
            ))}
          </nav>
